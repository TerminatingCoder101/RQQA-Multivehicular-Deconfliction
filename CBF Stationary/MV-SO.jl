...
function run_simulation()
    ...
    hist_control = [[0.0, 0.0]] # Init

    # Define two stationary obstacles
    obstacles = [
        Vehicle([-7.0, 2.5, 0.0, 0.0]),
        Vehicle([-3, -2.5, 0.0, 0.0]),
        Vehicle([-1, 2.5, 0.0, 0.0]),
        Vehicle([5.0, 0, 0.0, 0.0])
    ]
    ...
    for i in 1:N_STEPS
        ...
        # Write this in the standard form l <= A_qp * u <= u_qp
        A = sparse([A_cbf; I(2)])
        l = [b_cbf; A_MIN; W_MIN]
        ...
        # Solve the QP
        model = OSQP.Model()
        OSQP.setup!(model; P=P, q=q, A=A, l=l, u=u, verbose=false, eps_abs=1e-4, eps_rel=1e-4)
...
function plot_and_animate(hist_state, hist_control, obstacles, goal)
    ...
    anim = @animate for i in 1:length(hist_state)
        ...
        plot!(p_anim, x_hist[i] .+ R_AGENT .* cos.(theta), y_hist[i] .+ R_AGENT .* sin.(theta),
              seriestype=:shape, fillalpha=0.3, lw=0, label="Agent", color=:blue, markersize=1)
