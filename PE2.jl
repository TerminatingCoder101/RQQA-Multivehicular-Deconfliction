...
function calc_proportional_nav_control(state1, state2)
    p1 = state1[1:2]
    p2 = state2[1:2]; psi2 = state2[3]; v2 = state2[4]

    # Line-of-sight (LOS) vector from pursuer to evader
    delta_p = p1 - p2
    
    # Relative velocity
    v1_vec = state1[4] * [cos(state1[3]), sin(state1[3])]
    v2_vec = v2 * [cos(psi2), sin(psi2)]
    delta_v = v1_vec - v2_vec

    # Rate of change of the LOS angle (lambda_dot)
    # Formula: (dx*vy_rel - dy*vx_rel) / (dx^2 + dy^2)
    range_sq = dot(delta_p, delta_p)
    if range_sq < 0.01 #Div of 0 avoid
        return [0.0, 0.0] 
    end
    lambda_dot = (delta_p[1] * delta_v[2] - delta_p[2] * delta_v[1]) / range_sq

    w_cmd = N_GAIN * lambda_dot

    # The pursuer needs all the speed it can get to cut the evader off.
    a_cmd = 2.0 * (V_MAX_2 - v2) # P-controller for speed
...
function run_simulation()
    ...
    for i in 1:N_STEPS
        ...
        u_n2_raw = calc_proportional_nav_control(state1, state2)
        push!(interceptionHist, [0.0, 0.0])
        u_n2 = clamp(u_n2_raw, [A_MIN, W_MIN], [A_MAX, W_MAX])
        a_n2 = u_n2[1]
